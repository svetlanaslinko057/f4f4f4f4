...
export const METRIC_TOOLTIPS = {
  ...
  authorityScore: 'Authority Score (0-1000): Reflects network position and trustworthiness. Based on: who follows this account (quality of followers), historical prediction accuracy, connection to verified smart money wallets, and consistency over time.',
...
// Authority score color mapping
export function getAuthorityColor(score) {
  if (score >= 800) return { bg: 'bg-green-500', text: 'text-green-600', bar: '#22C55E' };
  if (score >= 700) return { bg: 'bg-lime-500', text: 'text-lime-600', bar: '#84CC16' };
  if (score >= 600) return { bg: 'bg-yellow-500', text: 'text-yellow-600', bar: '#EAB308' };
  if (score >= 500) return { bg: 'bg-orange-500', text: 'text-orange-600', bar: '#F97316' };
...
export function calculateAuthorityScore(influence, engagement, confidence) {
  const inf = influence || 0.5;
  const eng = engagement || 0.5;
  const conf = confidence || 0.6;
  return Math.round((inf * 0.4 + eng * 0.35 + conf * 0.25) * 1000);
}
