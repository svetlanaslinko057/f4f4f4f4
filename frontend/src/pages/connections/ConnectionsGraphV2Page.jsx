...
import { Sliders, RotateCcw, MousePointer, ArrowLeft, Search, X, Users } from 'lucide-react';
...
export default function ConnectionsGraphV2Page() {
  ...
  const nodeCanvasObject = useCallback((node, ctx, globalScale) => {
    ...
    const radius = isBacker ? 8 : isSearchTarget ? 10 : 5;
    ...
    const label = (node.label || node.handle || node.id).slice(0, 15);
    ctx.fillText(label, node.x, node.y + finalRadius + 2);
  ...
  const linkWidth = useCallback((link) => {
    ...
    return 1 + clamp01(link.weight || 0.1) * 2;
  }, [highlightSet]);
      ...
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Explore connections between influencers, VCs, and projects
          </p>
