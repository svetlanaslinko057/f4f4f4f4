...
import { Network, RefreshCw, AlertTriangle, Users, Filter, Info, ChevronDown, ChevronUp, HelpCircle, Shield, Skull, Eye, ExternalLink, ZoomIn, ZoomOut, Maximize2 } from 'lucide-react';
...
export default function FarmNetworkPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetchFarmGraph({ minScore, limit: 100 });
  ...
  const paintNode = useCallback((node, ctx, globalScale) => {
    ...
    const finalRadius = isSelected ? radius + 3 : radius;
    ...
    ctx.fillText(node.label, node.x, node.y + finalRadius + 2);
  ...
  // Link width based on overlap score
  const linkWidth = useCallback((link) => {
    return 1 + (link.score || 0.5) * 4;
  }, []);
