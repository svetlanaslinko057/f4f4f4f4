...
/**
 * Get user's following list (who they follow)
 * POST /following/:username
 * Returns list of accounts the user follows - for building follow graph
 */
app.post('/following/:username', async (req, res) => {
  const { username } = req.params;
  const { limit = 50, cookies, proxyUrl, userAgent } = req.body;

  if (!cookies || !Array.isArray(cookies)) {
    return res.status(400).json({ ok: false, error: 'Missing cookies' });
  }

  console.log(`[API] Following list: @${username} | Limit: ${limit}`);

  try {
    const result = await twitterClient.getFollowingList({
      username,
      limit,
      cookies,
      proxyUrl,
    ...
    // Note: DB saving is handled by backend, not parser
    // This keeps parser stateless and allows backend to use correct account IDs
    ...
    console.error(`[API] Following list error:`, error.message);
    res.status(500).json({ ok: false, error: error.message });
  }
});

/**
 * Get user's followers list (who follows them)
 * POST /followers/:username
 * Returns list of accounts that follow the user - reverse direction
 */
app.post('/followers/:username', async (req, res) => {
  const { username } = req.params;
  const { limit = 50, cookies, proxyUrl, userAgent } = req.body;
  ...
  console.log(`[API] Followers list: @${username} | Limit: ${limit}`);

  try {
    const result = await twitterClient.getFollowersList({
      username,
      limit,
      cookies,
      proxyUrl,
    ...
    console.error(`[API] Followers list error:`, error.message);
    res.status(500).json({ ok: false, error: error.message });
  }
});
...
// ========================================
// NARRATIVE INTEGRATION WEBHOOK
// ========================================

const BACKEND_WEBHOOK_URL = process.env.BACKEND_WEBHOOK_URL || 'http://localhost:8001/api/admin/twitter-parser/integration/webhook';

/**
 * Send tweets to backend for narrative detection
 */
async function sendToNarrativeDetection(tweets: any[], source: string): Promise<void> {
  ...
  try {
    const response = await fetch(BACKEND_WEBHOOK_URL, {
      ...
      body: JSON.stringify({ tweets, source }),
    });

    if (response.ok) {
      console.log(`[Webhook] Sent ${tweets.length} tweets to narrative detection`);
    } else {
      console.error(`[Webhook] Failed to send tweets: ${response.statusText}`);
    }
  } catch (err: any) {
    console.error(`[Webhook] Error sending tweets:`, err.message);
  }
}

/**
 * Search tweets with automatic narrative detection
 * POST /search-with-detection/:keyword
 */
app.post('/search-with-detection/:keyword', async (req, res) => {
  const { keyword } = req.params;
  const { limit = 20, cookies, proxyUrl, userAgent } = req.body;
  ...
  console.log(`[API] Search with detection: "${keyword}"`);
  ...
  try {
    ...
    // Send to narrative detection in background
    sendToNarrativeDetection(result.tweets || [], `search:${keyword}`);
...
async function start() {
  try {
    ...
    // Set db reference for follow graph
